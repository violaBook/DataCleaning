'''
functions-module[4-4]: 【值】
    EG: firstly cluster, then use median values to fill the clusters
    中文：先聚类，后对每个聚类类别的数据用其均值填补空值
demonstration:
     you can run like this:
        _____________________________you can run like this____________
    |   python fill4_kmeans_median.py                                      |____________
    | python fill4_kmeans_median.py './../new_data.pkl' 'pkl/fill4_kmeans_median.pkl'                |                                           |
...
class Same_Median_Fill():
    ...
    def get_data_zs(self):
        ...
        return data_zs
    # KMeans聚类,返回预测值
    def KMeansCluster(self, data_zs, k, b):
        # 预处理后数据数据
        data_zs_tmp = data_zs
        # 预测
    ...
    # 缺失值填补(按照聚类类别依次处理)
    def same_median_fill(self, r, k,  strategy):
        ...
        imp_median = SimpleImputer(missing_values=nan, strategy=strategy)
        for i in range(k):
            # 定位
            data_typei = self.csv_data[self.nan_columns][self.idx_[i]] # data_typei = self.csv_data[self.nan_columns][r[u'聚类类别'] == i] 等价
            # 缺失值插补
            elem = imp_median.fit_transform(data_typei) # 所有聚类fit后的元素值
            # p_tmp_list的赋值
            elem_T = elem.T
    ...
    def function(self):
        ...
        self.same_median_fill(r, self.k, self.strategy)
        # 写回数据库
        self.csv_data.to_pickle(self.write_file_path)
        # 其他
        profiler.stop()
